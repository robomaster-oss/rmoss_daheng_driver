...
: _camera_sn(camera_sn), _node(node), _config_path(config_path), _lut_path(lut_path), _lut_detail(
    lut_detail)
{
...
bool DaHengCam::grab_image(cv::Mat & image)
{
  ...
  VxInt32 DXstatus = DxRaw8ImgProcess(
    this->_pFrameBuffer->pImgBuf, image.data, this->_pFrameBuffer->nWidth,
    this->_pFrameBuffer->nHeight,
...
bool DaHengCam::load_config(const std::string & path)
{
  ...
  if (path != "") {
    ...
    // TODO(Holakk): 手动配置参数
    GXstatus = GXSetFloat(
      this->_hDevice, GX_FLOAT_EXPOSURE_TIME,
      2000.);
...
bool DaHengCam::load_lut(const std::string & path)
{
  ...
  GX_STATUS GxStatus = GXGetInt(
    this->_hDevice, GX_INT_COLOR_CORRECTION_PARAM,
  ...
  DXstatus = DxCalcCCParam(
    nColorCorrectionParam, 80,
  ...
  if (path != "") {
    ...
    // TODO(Holakk): 确定Lut表长度和载入方式正确: 生成的Lut表和getLut接口不一致
    RCLCPP_FATAL(
      this->_node->get_logger(),
      "ERROR:[%d] Read lut file failed!", this->_stClrImageProc.nLutLength);
    inFile.read(
      reinterpret_cast<char *>(&this->_stClrImageProc.pProLut),
      sizeof(uint8_t) * this->_stClrImageProc.nLutLength);
    inFile.close();
  } else {
    // 手动配置lut表参数 默认采用
    // 对比度调节参数[-50-100] 伽马调节参数[0.1-10] 亮度调节参数[-150-150]
    DXstatus = DxGetLut(
      this->_lut_detail[0], this->_lut_detail[1], this->_lut_detail[2],
      this->_stClrImageProc.pProLut,
      &this->_stClrImageProc.nLutLength);
    if (DXstatus != DX_OK) {
      if (this->_stClrImageProc.pProLut != NULL) {
        delete[] this->_stClrImageProc.pProLut;
